...
def parse(opt_path, is_train=True):
    ...
    if "train" in opt:
        ...
        if "T_period_rel" in opt["train"]:
            opt["train"]["T_period"] = [
                int(x * niter) for x in opt["train"]["T_period_rel"]
            ]
        if "restarts_rel" in opt["train"]:
            opt["train"]["restarts"] = [
                int(x * niter) for x in opt["train"]["restarts_rel"]
            ]
        if "lr_steps_rel" in opt["train"]:
            opt["train"]["lr_steps"] = [
                int(x * niter) for x in opt["train"]["lr_steps_rel"]
            ]
        if "lr_steps_inverse_rel" in opt["train"]:
            opt["train"]["lr_steps_inverse"] = [
                int(x * niter) for x in opt["train"]["lr_steps_inverse_rel"]
            ]
        print(opt["train"])
