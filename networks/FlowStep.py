...
import networks
import networks.Permutations
from networks import flow, thops, FlowAffineCouplingsAblation, StdConditional
from utils.util import opt_get


def getConditional(rrdbResults, position):
    img_ft = (
        rrdbResults if isinstance(rrdbResults, torch.Tensor) else rrdbResults[position]
    )
    return img_ft
...
class FlowStep(nn.Module):
    FlowPermutation = {
        ...
        "resqueeze_invconv_alternating_2_3": lambda obj, z, logdet, rev: obj.invconv(
            z, logdet, rev
        ),
        "resqueeze_invconv_3": lambda obj, z, logdet, rev: obj.invconv(z, logdet, rev),
        "InvertibleConv1x1GridAlign": lambda obj, z, logdet, rev: obj.invconv(
            z, logdet, rev
        ),
        "InvertibleConv1x1SubblocksShuf": lambda obj, z, logdet, rev: obj.invconv(
            z, logdet, rev
        ),
        "InvertibleConv1x1GridAlignIndepBorder": lambda obj, z, logdet, rev: obj.invconv(
            z, logdet, rev
        ),
        "InvertibleConv1x1GridAlignIndepBorder4": lambda obj, z, logdet, rev: obj.invconv(
            z, logdet, rev
        ),
    }
    ...
    def __init__(
        ...
        # 1. actnorm
        self.actnorm = networks.FlowActNorms.ActNorm2d(in_channels, actnorm_scale)

        # 2. permute
        if flow_permutation == "invconv":
            self.invconv = networks.Permutations.InvertibleConv1x1(
        ...
        if flow_coupling == "CondAffineSeparatedAndCond":
            self.affine = (
                networks.FlowAffineCouplingsAblation.CondAffineSeparatedAndCond(
                    in_channels=in_channels, opt=opt, std_mode=self.std_mode
                )
            )
    ...
    def normal_flow(self, z, logdet, rrdbResults=None, std=None):
        ...
        # 3. coupling
        if need_features or self.flow_coupling in [
            "condAffine",
            "condFtAffine",
            "condNormAffine",
        ]:
            ...
            z, logdet = self.affine(
                input=z, logdet=logdet, reverse=False, ft=img_ft, std=std
            )
    ...
    def reverse_flow(self, z, logdet, rrdbResults=None, std=None):
        ...
        if need_features or self.flow_coupling in [
            ...
            img_ft = getConditional(rrdbResults, self.position)
            z, logdet = self.affine(
                input=z, logdet=logdet, reverse=True, ft=img_ft, std=std
            )
