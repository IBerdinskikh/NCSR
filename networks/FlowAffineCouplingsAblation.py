...
from networks import thops
from networks.flow import Conv2d, Conv2dZeros, Linear, LinearZeros
...
class CondAffineSeparatedAndCond(nn.Module):
    def __init__(self, in_channels, opt, std_mode=True):
        ...
        hidden_channels = opt_get(
            opt, ["network_G", "flow", "CondAffineSeparatedAndCond", "hidden_channels"]
        )
        self.hidden_channels = 64 if hidden_channels is None else hidden_channels

        self.affine_eps = opt_get(
            opt, ["network_G", "flow", "CondAffineSeparatedAndCond", "eps"], 0.0001
        )
        ...
        self.fAffine_std = self.F(
            in_channels=self.channels_for_nn + self.std_channels,
            out_channels=self.channels_for_co * 2,
            hidden_channels=self.hidden_channels,
            kernel_hidden=self.kernel_hidden,
            n_hidden_layers=self.n_hidden_layers,
    ...
    def forward(
        self, input: torch.Tensor, logdet=None, reverse=False, ft=None, std=None
    ):
        if std is not None:
            h, w = input.size(2), input.size(3)
            if self.std_channels == 3 and std.size(1) == 3:
                std_expand = torch.nn.functional.interpolate(
                    std, size=(h, w), mode="bicubic"
                )
        ...
        if not reverse:
            ...
            if self.stdmode and std is not None:
                # Std Conditional
                z1, z2 = self.split(z)
                scaleFt, shiftFt = self.feature_extract_aff(
                    z1, std_expand, self.fAffine_std
                )
