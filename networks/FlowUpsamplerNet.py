...
import networks.Split
from networks import flow, thops
from networks.Split import Split2d
from networks.glow_arch import f_conv2d_bias
from networks.FlowStep import FlowStep
...
class FlowUpsamplerNet(nn.Module):
    def __init__(
        ...
        self.std_blocks = opt_get(opt, ["network_G", "flow", "S"])
    ...
    def arch_FlowStep(
        ...
        condStd = False
        for k in range(K):
            if self.std_blocks[L_level - 1] >= K - k:
                condStd = True
    ...
    def arch_split(self, H, W, L, levels, opt, opt_get):
        ...
        if (
            opt_get(opt, ["network_G", "flow", "split", "enable"])
            and L < levels - correction
        ):
            logs_eps = opt_get(opt, ["network_G", "flow", "split", "logs_eps"]) or 0
            consume_ratio = (
                opt_get(opt, ["network_G", "flow", "split", "consume_ratio"]) or 0.5
            )
            position_name = get_position_name(H, self.opt["scale"])
            position = (
                position_name
                if opt_get(opt, ["network_G", "flow", "split", "conditional"])
                else None
            )
    ...
    def arch_additionalFlowAffine(
        self, H, LU_decomposed, W, actnorm_scale, hidden_channels, opt
    ):
        if "additionalFlowNoAffine" in opt["network_G"]["flow"]:
            n_additionalFlowNoAffine = int(
                opt["network_G"]["flow"]["additionalFlowNoAffine"]
            )
    ...
    def forward(
        ...
        if reverse:
            ...
            z, logdet = self.encode(
                gt, rrdbResults, logdet=logdet, epses=epses, y_onehot=y_onehot, std=std
            )

            return z, logdet

    def encode(self, gt, rrdbResults, logdet=0.0, epses=None, y_onehot=None, std=None):
        fl_fea = gt
        reverse = False
        level_conditionals = {}
        bypasses = {}
        ...
        for layer, shape in zip(self.layers, self.output_shapes):
            ...
            # print(layer)
            # print(fl_fea.size())
            # print(level_conditionals[level].size())
    ...
    def decode(
        ...
        if (
            not opt_get(
                self.opt, ["network_G", "flow", "levelConditional", "conditional"]
            )
            == True
        ):
